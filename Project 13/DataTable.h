...
public:
	...
	{
		// add all numbers together using accumulate func
		T mean = std::accumulate(y.begin(), y.end(), 0.0);
		// divide sum of numbers by number count and return
		return mean / y.size();
	}
	T sigma() const // returns the standard deviation of the dependent coordinates
	{
		...
		// loop through vector y (dependent coordinates)
		// subtract mean from each dependent y coordinate then sqaure it, add it too the calculating sum
		for (int i = 0; i < y.size(); i++) {
			sum += ((y[i] - m) * (y[i] - m));
		}
		// now that we have the sum we can divide by n(size) - 1 and square root result for the standard deviation
	...
	void regression(T& slope, T& y_intercept) const // returns the slope and intercept for the data set
	{
		...
		yd += (x[i] - xAverage) * (y[i] - yAverage);
		xi += (x[i] - xAverage) * (x[i] - xAverage);
		}
